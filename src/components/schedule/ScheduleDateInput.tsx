...
export type ScheduleDateInputProps = {
  value: string;
  onChange: (value: string) => void;
  onBlur?: () => void;
  /** Called when a valid YYYY-MM-DD is chosen (typed blur or calendar). */
  onValidDate?: (ymd: string) => void;
  placeholder?: string;
  className?: string;
...
export function ScheduleDateInput({
  ...
  const ymd =
    parseScheduleDateInput(finalizeScheduleDateInput(value)) ??
    parseScheduleDateInput(value) ??
    '';
  ...
  return (
    <div className={cn('relative min-w-0', className)}>
      <input
        ...
        value={value}
        onChange={(e) => onChange(normalizeScheduleDateInput(e.target.value))}
        onBlur={onBlur}
      />
      <input
        ref={nativeRef}
        ...
        onChange={(e) => {
          ...
          if (!picked) return;
          onChange(formatScheduleDateInput(picked));
          onValidDate?.(picked);
        }}
      />
      <button
